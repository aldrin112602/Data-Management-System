...
@section('content')
    ...
    <form action="{{ route('location-data.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        <div class="mb-4">
            <label for="location_type" class="block text-gray-700">Location Type</label>
            <input type="text" name="location_type" id="location_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
        </div>

        <div class="mb-4">
            <label for="location_logo" class="block text-gray-700">Location Logo</label>
            <input type="file" name="location_logo" id="location_logo" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
        </div>
